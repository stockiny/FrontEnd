...
export class CategorylistComponent {
  ...
  deleteBtn(id) {
   ...
   swalWithBootstrapButtons.fire({
    ...
    if (result.isConfirmed) {
      this.appService.deteleCategory(id)
  .subscribe(response => {
  ...
  private getTableData(pageOption: pageSelection): void {
    this.appService.getCategories().subscribe((apiRes) => {
      this.categoriesData = [];
      this.tableData = [];
      this.serialNumberArray = [];
      this.totalData = apiRes.categories.length;
      apiRes.categories.map((res: CategoriesModel, index: number) => {
